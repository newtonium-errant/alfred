...
- `shadow_capture.enabled: false` on **both** — Salem-only measurement infra for KAL-LE; **load-bearing PHI exclusion** for Hypatia (Groq raw-PCM egress; `config.yaml.example` explicitly forbids it on PHI/sovereign instances). `endpoint_hold` is safe everywhere (features-only telemetry, no raw-transcript egress). *(Why Hypatia is named — do NOT sweep this on "Hypatia isn't clinical": all clinical work is STAY-C's, and this exclusion is defence-in-depth after a REAL 2026-07-08 PHI-egress incident, not a claim about Hypatia's workload. It targets REAL crossover, not incidental mention, which is an accepted residual. Full note: "WHY THE SETTINGS NAMING HYPATIA STAY" in `config.yaml.example`, under `telegram.stt`.)*
