...
- **Long-running-daemon schedule drift.** We saw an ~16-minute early fire on the brief yesterday despite clock-aligned scheduling. Debugging under way — suspect the `asyncio.sleep` drift-compensation path. Happy to share the post-mortem if useful.
...
Happy to split any of the above into their own threaded replies below if useful. Everything here traces to commits on our `master`; the session notes bundle the reasoning alongside.
