"""Composition helper for the ``/today`` slash command (Tier Phase 2A,
2026-05-28).

The ``/today`` command is a Salem-only glance-view mini-brief: the same
three sections the morning brief surfaces at the top — tier (open tasks
by priority bucket), today's routines, and upcoming events — composed
into one Telegram reply. Read-only; no vault writes, no session record.
...
Why ``/today`` instead of just re-using the brief file:
  ...
  3. **Same render functions as the brief.** The composition imports
     ``brief.tier_section.render_tier_section``,
     ``brief.routine_section.render_routine_section``, and
...
(deadline-driven actionable), routines second (recurring practice
anchors), upcoming events third (forward-looking calendar). Each
section is preceded by its header text so the operator's mental model
of the brief carries over.

Section headers as plain Markdown ``## Header`` lines: the brief writes
``## Open Tasks by Tier`` etc. to the markdown file; the slash command
mirrors the same shape so the eye recognises the structure.

Telegram has a ~4096 character body limit. The composer caps the reply
defensively at 4000 characters; if all three sections together exceed
...
cases (40+ open tasks across all tiers + a busy routine day + a packed
calendar week).
"""
...
from alfred.brief.routine_section import (
    SECTION_HEADER as ROUTINES_SECTION_HEADER,
...
# driven) first, routines (recurring practice) second, upcoming events
# (forward calendar) third.
...
def compose_today_reply(
    ...
    Calls the three brief render functions in the canonical ordering
    (tier → routines → upcoming events) and joins the sections with
    Markdown ``## Header`` lines so the visual shape matches the
    morning brief's top sections.

    ``now`` is the reference instant for the tier section's deadline
    math + the date selection for routines + events. Caller (the bot
    handler) computes ``datetime.now(ZoneInfo(config.today_command.timezone))``.
    ...
    # Routines section — date-only.
    try:
        routines_body = render_routine_section(vault_path, today_local)
    except Exception as exc:  # noqa: BLE001
        log.warning(
            "today_command.routines_render_failed",
            error_type=exc.__class__.__name__,
            error=str(exc),
        )
        routines_body = "*(routines render failed; see brief log)*"
    ...
    composed = (
        ...
        f"{tier_body.rstrip()}\n\n"
        f"## {ROUTINES_SECTION_HEADER}\n\n"
        f"{routines_body.rstrip()}\n\n"
