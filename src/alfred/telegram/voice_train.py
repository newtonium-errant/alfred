...
# ---------------------------------------------------------------------------
#
# All four extraction prompts (voice-leaf / voice-cluster / voice-overall /
# method-leaf) are externalised under
# ``src/alfred/_bundled/skills/vault-hypatia/prompts/`` so prompt-tuner can
# iterate on them without touching this Python module. Loaded fresh per call
# (no module-import caching) — edits to the .md files take effect on the
# NEXT extraction without needing a daemon restart.
#
# ``method_extraction.md`` joined the externalised set on 2026-05-09 (Batch A
# substrate hygiene) once the voice prompts had been validated through one
# extraction cycle. The prior comment carving METHOD_EXTRACTION_PROMPT out
# of scope is now superseded — same loader, same per-call freshness contract.


def _load_voice_prompt(prompt_file: str) -> str:
    """Load an extraction prompt from the bundled vault-hypatia/prompts/ dir.

    Despite the name (kept for backward compat with the existing voice
    helpers), this loader is generic over any ``.md`` file living under
    ``vault-hypatia/prompts/`` — voice and method prompts share the
    importlib.resources-located parent dir and identical fresh-read
    semantics. Mirrors ``alfred.distiller.pipeline._load_stage_prompt``:
    importlib.resources locator, fresh read per call, warning + empty
    string on missing file.
    """
...
def get_method_extraction_prompt() -> str:
    """Return the method-leaf extraction prompt (read fresh per call)."""
    return _load_voice_prompt("method_extraction.md")
...
async def extract_method_profile(
    *,
    ...
    return await _call_opus(
        client=client,
        model=model,
        system_prompt=get_method_extraction_prompt(),
