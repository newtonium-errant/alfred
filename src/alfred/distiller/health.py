...
def _read_last_error(state_path: Path) -> dict | None:
    """Read distiller state and return the ``last_error`` payload
    (shape: ``{"ts": iso_string, "message": str}``) or None when
    absent / unreadable / corrupted-shape.

    Same defensive-read posture as ``_read_distiller_most_recent_run``
...
def _check_last_successful_extraction(raw: dict[str, Any]) -> CheckResult:
    ...
    line carries WHY the extraction stalled. OK detail stays clean —
    ...
    # successful extraction would only happen if an operator hand-
    # edited the state file, which the defensive _read_last_error has
    ...
    if elapsed < timedelta(hours=_DISTILLER_STALE_FAIL_HOURS):
        return CheckResult(
            name="last-successful-extraction",
            status=Status.WARN,
            detail=f"last extraction {round(elapsed_hours, 1)}h ago (one missed run){error_suffix}",
            data=payload,
        )
    return CheckResult(
        name="last-successful-extraction",
        status=Status.FAIL,
        detail=f"last extraction {round(elapsed_hours, 1)}h ago (daemon may be silently failing){error_suffix}",
        data=payload,
    )
