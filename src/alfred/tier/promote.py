...
B1 (this module) is DETECTION + the pending/decided store, PROPOSE-ONLY: it writes ONLY the pending
proposal queue — it NEVER creates or edits a ``routine/`` record (that is B2's approval-gated write).
learn (done_at history) → propose (pending queue) → operator-approves (B2). The judgment path is made
safe by: a THRESHOLD (≥ N distinct done-days in a window), PROPOSE-ONLY, and exclusions (an item
already in a routine, already pending, or already decided is never re-proposed).

Mirrors the ``routine.match_calibration`` pending/glossary pattern: schema-tolerant JSONL twins that
degrade-not-crash on a corrupt row; the DECIDED store is DEFINED + READ here (the re-proposal
exclusion) and WRITTEN by B2's approval CLI/reply.
"""
...
# Ad-hoc T3 sources eligible for recurrence promotion — a free-text chore the operator typed, NOT a
# routine-origin surfaced item (which already lives in a routine record). See daily_curation SOURCES.
_ADHOC_T3_SOURCES: frozenset[str] = frozenset({"operator-adhoc", "aspirational", "operator", "rollover"})
