...
class Embedder:
    ...
    async def process_diff(
        ...
        if self._is_cloud:
            await self._process_diff_cloud(
                to_embed, records,
            ...
            await self._process_diff_local(
                to_embed, records,
                failed_embed=failed_embed,
                parse_errors=parse_errors,
                empty_skipped=empty_skipped,
    ...
    async def _process_diff_local(
        ...
        to_embed: list[str],
        records: dict[str, VaultRecord],
        failed_embed: list[str] | None = None,
        parse_errors: list[str] | None = None,
        empty_skipped: list[str] | None = None,
    ) -> None:
        """Original one-at-a-time embed loop for local Ollama."""
        if failed_embed is None:
            failed_embed = []
        if parse_errors is None:
            parse_errors = []
        if empty_skipped is None:
            empty_skipped = []
