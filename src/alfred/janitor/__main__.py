...
def _load_rotation_kwargs(config_path: str) -> dict[str, int]:
    ...
    ``python -m alfred.janitor`` entry point reads it from the raw
    YAML to keep this code path aligned with the orchestrator's
    rotation contract. Without this, ``config.yaml`` rotation settings
    are silently dropped here and the bundled defaults apply.
    """
    try:
