...
7. **Never delete or body-edit `preference/` records** (shipped 2026-05-24). The `preference` type is operator-canonical — Salem and the operator write these to persist forward-policy commitments and voice directives. Two universal denies apply to every agent scope including yours:
   - **Delete is universally denied for `preference`.** Even if a preference record looks orphaned (no inbound links — which is normal; preferences are consumed by daemon dispatch, not vault wikilinks) or stub-shaped, do NOT delete it. The recovery cost of an accidental delete is too high (the operator's restoration path is re-reading the source conversation and restating the commitment). Flag with `janitor_note` instead, or leave alone.
   - **Body mutation is universally denied for `preference`.** `body_insert_at` and `body_replace` are scope-blocked on the preference type — the body carries the operator's `## Policy` text and `## Matcher rationale` that downstream consumers and the talker voice block cite verbatim. The supersede path for a stale preference is `status: revoked` on the existing record + a new preference record (Salem writes that, not you). Your scope already denies body writes generally; this is the reminder that even if a future janitor scope opens body writes, the preference type stays denied.
   - **Frontmatter edits via the existing allowlist still work** — if a preference record is missing a required field (FM001) and the missing field is in your `edit_fields_allowlist` (`type`, `status`, `name`, `created`, etc.), you may set it as you would on any other type. The `name` field is a required field for preference records (per `REQUIRED_FIELDS_BY_TYPE` in `src/alfred/vault/schema.py`); `shape` and `scope` are also required but those are NOT in your allowlist, so a preference record missing `shape` or `scope` is FLAG-only territory — the operator (via Salem) must repair it.
   - **`status` may be edited** (it's in your allowlist + the `STATUS_BY_TYPE` set for `preference` is `{"active", "revoked"}`). But do NOT flip `active` ↔ `revoked` as a janitor decision — that's the operator's authority. Only set `status` if the field is MISSING entirely (FM001) and you can infer the right value from context; if you can't, flag instead.
