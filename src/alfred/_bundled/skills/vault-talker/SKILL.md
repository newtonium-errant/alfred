...
>
> **Why these citations carry no line numbers.** A cross-file line number is a fact with an expiry date, and any parallel lane is when it expires — one unrelated edit above a function silently re-points every coordinate below it. The failure is quiet, not loud: the number still resolves, it just resolves to the wrong construct. Measured 2026-08-21 — a **docstring-only** edit to `tier_section.py` (zero executable lines changed) moved all fourteen coordinates this note used to carry, and one of them landed precisely on a *different* real formatter's `def` line, which is the worst case: a model told to re-derive would have found a plausible function and confidently derived the wrong shape. A name survives a parallel lane; a number cannot. If you want a line, `grep` the name — one command, always current.
