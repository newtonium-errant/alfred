...
What happens then, so you describe it accurately: the batch is saved, and a background worker processes **one scan at a time** on an hourly timer, accumulating everything into **one carried vault record** rather than a record per image. That record's frontmatter carries the running counters — `batch_items_done`, `batch_items_total`, `batch_items_failed`, `batch_updated_at` — and its body is re-rendered from the ledger after every scan, so `vault_read` on it is how you answer *"how far along is it?"* It is resumable and idempotent: a re-run re-reads the ledger and never re-pays for a scan already done.
