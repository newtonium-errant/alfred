...
>
> Salem (internal): "rake leaves" is short, generic, cadence-less — not proper-noun task-shaped, and not an obvious recurring practice. Per the discrimination rule, task-search first: `vault_search glob="task/Rake*"` → no `task/` record. Then `routine_done item="rake leaves"` (vault-wide fuzzy) → `{"kind": "unknown_item", ...}` — not a routine item either.
>
