...
>
> **Why these citations carry no line numbers.** A cross-file line number is a fact with an expiry date, and any parallel lane is when it expires — one unrelated edit above a function silently re-points every coordinate below it. The failure is quiet, not loud: the number still resolves, it just resolves to the wrong construct. Measured 2026-08-21 against a **docstring-only** edit to `tier_section.py` — zero executable lines changed, AST-proven — which still moved all **11** coordinates this note carried, and all **14** the SKILL carried into that file. Twice, a stale coordinate landed exactly on a *different real formatter's* `def` line. That is the worst case, and it is why renumbering is not the fix: a model told to re-derive finds a plausible function and confidently derives the wrong shape, with nothing anywhere reading as broken. The same lane then moved them **again** in a single gate round, so renumbering would have gone stale within hours. A name survives a parallel lane; a number cannot. If you want a line, `grep` the name — one command, always current.
