...
>
>   - **Schema-level** ("does `tier_curation` carry a rollover anchor?") — answerable from source / SKILL prior context. The answer is yes: `rollover_from` is an optional ISO-date field per `alfred.tier.daily_curation.DailyCuration.rollover_from`.
...
>   2. `vault_search grep="rollover"` → surfaces the actual implementation site (`alfred.brief.tier_section.compute_rollover` + `_render_unplaced_carryover`, NOT the aggregator).
>
