...
class ClaudeBackend(BaseBackend):
    ...
    async def process(
        ...
        if proc.returncode != 0:
            # Classify + summarise via the shared helper so quota / auth /
            # other is derived from the SAME real failing traffic everywhere
            # (zero marginal tokens). stderr-only summaries went content-free
            # during the 2026-07-29 weekly-limit outage — the quota banner
            # lands on stdout.
