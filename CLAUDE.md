...
**Default stays the plain mailbox** (drain your inbox once at session start, then only on natural turns). Live-mode is **opt-in and scoped to the session it's requested in** — nothing persists across sessions. It needs BOTH sides live (operator triggers it in each session independently); if only one side is live, route-on-send still lands the message and the non-live peer reads it on its own next turn (worst case = today's latency). Do not enter it unprompted.

**The bus is lossy, and it drops silently (2026-08-19).** Drops are confirmed in BOTH directions, one message eaten three times, with no error raised at the sender — which retires the "zero loss" claim this section used to carry, and note that a send returning success is evidence the send was ACCEPTED, never that anything arrived. Two consequences to design around. (1) **A notification is load-bearing, not a courtesy** — a delta-confirm that never lands leaves already-published figures standing as though confirmed, so a dropped message yields a wrong conclusion rather than a visibly missing one. (2) **Coordinate on authoritative state, not on increments** — number your messages, carry the whole current state in each rather than a diff against the last, resolve disagreement later-timestamp-wins, and verify any condition you are about to ACT on from the TREE (`git rev-parse`, `git log`, the file itself) rather than from your inbox. `.claude/live-coordination-mode.md` still carries the retired claim in its both-sides-degradation section ("Delivery already happened"; "Zero message loss"); this paragraph is the current one until that file is corrected.
