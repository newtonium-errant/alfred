...
## Capability claims — your two clauses from the CLAUDE.md SOP

`CLAUDE.md`'s **SOP — Capability-claim audit** governs assertions about what the system *can do*. **You do not read `CLAUDE.md`** (see its own reading-list map), so the two clauses that assign *your* behaviour are mirrored here. The rest of the SOP — the tiering, the dispatch question, the Tier-2 gate — is team-lead/builder/reviewer work and lives only there.

1. **Cite CONSTRUCTS, never coordinates — scoped to bundles whose instance cannot verify a line number.** A cross-file coordinate is a fact with an expiry date: one in `vault-talker` cited `src/alfred/cli.py:2497-2514` — the mail-fetch block — and was correct when written on 2026-06-06 (`f6d3d49f`). No edit ever touched the citation; the file simply grew above it. **Two figures, each with its window, and neither substitutes for the other:** ~**104 lines** within five days (`total = fetch_all(...)` 2502 → 2606 at `f79692d3`, authored 2026-06-11 — the ONLY commit touching `cli.py` in that range, so the crossing is bisected, not sampled), and ~**2000 lines** across two and a half months (`time.sleep(config.poll_interval)` 2509 → 4512 at `08d7b991`), by which point the coordinate `2497-2514` lands inside `cmd_instance` — a *different command handler*. Quote whichever matches the window you mean, never one under the other's window. **The short-window figure is the more useful warning:** ~100 lines of ordinary work is already enough to aim a coordinate at the wrong function, so a reader who hears only "2000" is handed a safe case that does not exist. Name the construct; `tests/test_talker_skill_derivation_pin.py` pins it in three spellings and you own both the claim and the pin. **This is NOT uniform across bundles:** `vault-kalle/SKILL.md` deliberately instructs the opposite ("cite file paths and line numbers when you've read code") because KAL-LE **has** `bash_exec` and can check. Salem does not. Extend the rule only where the reader cannot verify — and **sweep a bundle to zero before widening the pin to cover it**, or it ships red on arrival and trains people to ignore it.

2. **Stamp what cannot be checked either way.** A claim about perishable operator/deployment state — "no `brief:` block on that instance", "exactly one topic MOC" — carries a **date** and an explicit instruction to believe the operator over the paragraph. You cannot verify deployed state (it needs SSH; that is the team-lead's) — and when you read `./config*.yaml` you are reading the **dev tree**, so say so explicitly rather than reporting it as deployment truth. A gate ruled a shipped SKILL claim false on exactly that read and had to withdraw it. Stamping does not catch staleness; it stops a stale claim being *read as current*, which is most of the harm.

**Your rule 6 above is the SOP's second trap** and has been promoted to `CLAUDE.md` verbatim-by-reference because team-lead does not read this file. It is stated once, here. The SOP's addition: score a false justification by **the inferences it licenses**, not by its distance from the truth — "it survived by luck" implies a safe case the true mechanism does not have.
