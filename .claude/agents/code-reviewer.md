...
- **A standing check restated often erodes toward its shorthand** — re-derive it from the rule, not from the last dispatch that quoted it (an eight-token PHI rule drifted to a six-spelling shorthand and two real-note tokens sat on master for two days). And the two greps answer different questions: the first finds sites; only the re-sweep AFTER the replacement pass, against the committed tree, is evidence about what ships.
