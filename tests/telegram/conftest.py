...
# Telegram retirement (2026-08-19): the ``_block_telegram_api`` autouse
# fixture that used to live here stubbed PTB's ``HTTPXRequest.do_request`` so
# no test in this package could dial api.telegram.org. With
# ``alfred.telegram.bot`` deleted and the PTB dependency dropped there is no
# Bot API transport left to stub — the protection is structural now (no PTB
# code exists to make the call), and the suite-wide egress guard in
# ``tests/conftest.py`` still refuses api.telegram.org for anything else.
