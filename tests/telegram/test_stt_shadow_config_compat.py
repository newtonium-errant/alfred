...
carries ``telegram: stt: shadow_capture:`` would raise ``TypeError`` at load
the moment the field stopped existing — taking down every talker-config
consumer, including ``alfred.telegram.health``, whose import failure is
SWALLOWED by ``health/aggregator._load_tool_checks`` and would silently drop
the talker probe from the registry rather than fail loudly. The field is a
config-compat shell, exactly like ``TodayCommandConfig`` and
``InstanceConfig.aliases`` kept by T5 (feca775f / 656d1a87).
